...
public class CodeChallengeBuilder implements SQLEntityBuilder<CodeChallenge> {

	@Override
	public CodeChallenge fromResultSet(ResultSet sqlResults) {
		try {
			...
			return new CodeChallenge(clientId, codeChallenge, state, expires);
		} catch(Exception e) {
			e.printStackTrace();
			return null;
		}
